"""Conditional rectified-flow velocity-field training loop.
...
from source.flow_matching.flow_matching_cfg import FlowMatchingCfg
...
def _flow_matching_loss(
  ...
  """Rectified-flow L1 loss for the straight data-to-noise probability path."""
  B = x_data.shape[0]
  K = num_noise_samples
  x_data_exp = x_data[:, None].expand(B, K, *x_data.shape[1:]).reshape(
    B * K, *x_data.shape[1:]
  )
  time = torch.rand(B * K, device=x_data.device, dtype=x_data.dtype)
  noise = torch.randn_like(x_data_exp)
  time_view = time.view(B * K, 1, 1)
  x_t = (1.0 - time_view) * x_data_exp + time_view * noise
  target_velocity = noise - x_data_exp
  ...
  return F.l1_loss(
    model(
      x_t, time * time_embedding_scale, terrain=terrain_exp, proprio=proprio_exp
    ),
    target_velocity,
  )
...
def train_flow_matching(cfg: FlowMatchingCfg) -> Path:
  """Train a conditional rectified-flow velocity predictor."""
  ...
  for epoch in range(cfg.num_epochs):
    ...
    for batch in train_loader:
      ...
      loss = _flow_matching_loss(
        model, x_0, cfg.num_noise_samples, cfg.time_embedding_scale, terrain, proprio
      )
    ...
    if epoch % cfg.log_interval == 0:
      ...
      val_loss = _validate(
        eval_model, val_loader, device, pin_memory, cfg.num_noise_samples,
        cfg.time_embedding_scale,
...
def _validate(
  ...
  for batch in val_loader:
    if has_terrain:
      ...
      proprio = None
    x_0 = x_0.to(device, non_blocking=pin_memory)
    total += _flow_matching_loss(
      model, x_0, num_noise_samples, time_embedding_scale, terrain, proprio
    )
    n += 1
...
if __name__ == "__main__":
  train_flow_matching(tyro.cli(FlowMatchingCfg))
