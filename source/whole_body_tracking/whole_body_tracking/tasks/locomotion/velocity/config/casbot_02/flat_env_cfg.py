...
class CASBOTLocomotionFlatEnvCfg(LocomotionVelocityFlatEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.rewards.create_joint_deviation_l1_rewterm(
            "joint_deviation_torso_l1", -0.1, ["waist_yaw_joint"]
        )
        ...
        if self.__class__.__name__ == "CASBOTLocomotionFlatEnvCfg":
            self.disable_zero_weight_rewards()


@configclass
class CASBOTLocomotionFlatNoDRCfg(CASBOTLocomotionFlatEnvCfg):
    """CASBOT flat-terrain locomotion without domain randomization or observation noise."""

    def __post_init__(self):
        super().__post_init__()
        self.disable_domain_randomization()
        if self.__class__.__name__ == "CASBOTLocomotionFlatNoDRCfg":
            self.disable_zero_weight_rewards()
