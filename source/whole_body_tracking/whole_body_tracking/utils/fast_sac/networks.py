...
class Actor(nn.Module):
    """Stochastic policy network with tanh-squashed Gaussian output for SAC.

    Takes a flat observation tensor and outputs (action, mean, log_std).
    Actions are optionally squashed through tanh and scaled by action_scale + action_bias.
    """
...
class DistributionalQNetwork(nn.Module):
    """Single distributional Q-network that outputs logits over a discrete value support.

    Takes concatenated (obs, action) and returns num_atoms logits per sample.
    """
...
class Critic(nn.Module):
    ...
    Returns stacked logits [num_q, batch, num_atoms] and provides get_value() via support-weighted sum.
    """
